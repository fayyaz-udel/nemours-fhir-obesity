...
class DL_models():
    ...
    def dl_test(self):
        ...
        for i in range(1):
            ...
            self.net=torch.load('saved_models/obsNew_0.tar')
            print("[ MODEL LOADED ]")
            print(self.net)
...
#             print("contri",contri.shape)
            ...
            ind_contri=ind_contri[~ind_contri['feat'].isin(cond_not)]
...
#         print(auroc_mat)
    ...
    def display_contri_feat(self,ind_imp,title):
        ...
        #print(ind_imp['feat'].unique())
        #print(ind_imp)
        ind_imp=ind_imp.groupby('feat').agg({'impmean':['mean'],'impcount':'sum'}).reset_index('feat')
        #print(ind_imp)
        ind_imp.columns=['feat','impmean','impcount']
        n=int(ind_imp.shape[0]/2)
        print(ind_imp)
        
        
        ind_imp_all=ind_imp.sort_values(by='impmean',ascending=False)
        ...
        ind_imp=ind_imp.sort_values(by='impcount',ascending=False)
        print(ind_imp)
        ind_imp_high=ind_imp[:20]
        ind_imp_high=ind_imp_high.sort_values(by='impmean',ascending=True)
        ind_imp_high=ind_imp_high[ind_imp_high['feat']!=0]
        #print("hi",ind_imp_high)
        tot=ind_imp_high['impmean'].sum()
        ind_imp_high['impmean']=ind_imp_high['impmean']/tot
        #print(ind_imp_high)
        
        plt.rcParams["figure.figsize"] = (10,20)
        ind_imp_high["label"] = ind_imp_high["feat"].astype(str)
        plt.barh(ind_imp_high['label'], ind_imp_high['impmean'])
        plt.xticks(fontsize=20)
        plt.yticks(fontsize=20)
        #plt.axhline(y=0)
        plt.title(title, fontsize=20)
        ...
        ind_imp_mid=ind_imp_mid.sort_values(by='impmean',ascending=True)
        ind_imp_mid=ind_imp_mid[ind_imp_mid['feat']!=0]
        tot=ind_imp_mid['impmean'].sum()
        ind_imp_mid['impmean']=ind_imp_mid['impmean']/tot
        
        plt.rcParams["figure.figsize"] = (10,20)
        ind_imp_mid["label"] = ind_imp_mid["feat"].astype(str)
        plt.barh(ind_imp_mid['label'], ind_imp_mid['impmean'])
        plt.xticks(fontsize=20)
        plt.yticks(fontsize=20)
        #plt.axhline(y=0)
        plt.title(title, fontsize=20)
        ...
        ind_imp_low=ind_imp_low.sort_values(by='impmean',ascending=True)
        ind_imp_low=ind_imp_low[ind_imp_low['feat']!=0]
        tot=ind_imp_low['impmean'].sum()
        ind_imp_low['impmean']=ind_imp_low['impmean']/tot
        
        plt.rcParams["figure.figsize"] = (10,20)
        ind_imp_low["label"] = ind_imp_low["feat"].astype(str)
        plt.barh(ind_imp_low['label'], ind_imp_low['impmean'])
        plt.xticks(fontsize=20)
        plt.yticks(fontsize=20)
        #plt.axhline(y=0)
        plt.title(title, fontsize=20)
    ...
    def display_contri(self,ind_imp,title):
        ...
        tot=ind_imp_all['impmean'].sum()
        ind_imp_all['impmean']=ind_imp_all['impmean']/tot
        #ind_imp_all.loc[-1] = ["Family History",0.00913,0] 
        #ind_imp_all=ind_imp_all.reset_index()
        ind_imp_all.loc[-2] = ["Demographics",0.03213,0] 
        ind_imp_all=ind_imp_all.sort_values(by='impmean',ascending=True)
        print(ind_imp_all)
    ...
    def display_contri_old(self,ind_imp,title):
        n=int(ind_imp.shape[0]/2)
        ind_imp=ind_imp.sort_values(by='impcount',ascending=False)
